...
void Interpolator::Preprocess_multipleinput(const std::vector<cv::Mat>& img,
	std::vector<cv::Mat>* input_channels) {

	for (int i = 0; i < img.size(); i++)
	{
		img[i].copyTo((*input_channels)[i]);
	}
...
void Interpolator::Preprocess_multipleinput_GPU(const std::vector<cv::Mat>& img,
	std::vector<GpuMat>* input_channels)
{
	for (int i = 0; i < img.size(); i++)
	{
		GpuMat sample;
		sample.upload(img.at(i));
		sample.copyTo((*input_channels).at(i));
	}
...
Interpolator::Interpolator(const string& model_file, const string& trained_file, bool ifGPU) {
	...
	CHECK(num_channels_ == 3 || num_channels_ == 1 || num_channels_ == 4 )
...
std::vector<cv::Mat> Interpolator::interpolate(const cv::Mat& img) {
	...
	for (int i = 0; i*basesize < img.rows; i++)
	{
		for (int j = 0; j*basesize < img.cols; j++)
		{
			...
			cv::Mat_<float> tmpimg = cv::Mat(edi - sti, edj - stj, CV_32SC1);
			for (int i = sti; i < edi; i++)
			{
				for (int j = stj; j < edj; j++)
				{
					tmpimg(i - sti, j - stj) = img.at<float>(i, j);
				}
			}
...
std::vector<cv::Mat> Interpolator::interpolate_4x_c(const cv::Mat& img) {
	...
	for (int i = 0; i*basesize < img.rows; i++)
	{
		for (int j = 0; j*basesize < img.cols; j++)
		{
			...
			for (int i = sti; i < edi; i++)
			{
				for (int j = stj; j < edj; j++)
				{
					tmpsav1(i, j) = resmats[0].at<float>(i - sti + di, j - stj + dj);
					tmpsav2(i, j) = resmats[1].at<float>(i - sti + di, j - stj + dj);
					tmpsav3(i, j) = resmats[2].at<float>(i - sti + di, j - stj + dj);
					tmpsav4(i, j) = resmats[3].at<float>(i - sti + di, j - stj + dj);
					tmpsav5(i, j) = resmats[4].at<float>(i - sti + di, j - stj + dj);
					tmpsav6(i, j) = resmats[5].at<float>(i - sti + di, j - stj + dj);
					tmpsav7(i, j) = resmats[6].at<float>(i - sti + di, j - stj + dj);
					tmpsav8(i, j) = resmats[7].at<float>(i - sti + di, j - stj + dj);
					tmpsav9(i, j) = resmats[8].at<float>(i - sti + di, j - stj + dj);
					tmpsav10(i, j) = resmats[9].at<float>(i - sti + di, j - stj + dj);
					tmpsav11(i, j) = resmats[10].at<float>(i - sti + di, j - stj + dj);
					tmpsav12(i, j) = resmats[11].at<float>(i - sti + di, j - stj + dj);
				}
			}
...
std::vector<cv::Mat> Interpolator::interpolate_otherside(const std::vector<cv::Mat>& imgs)
{
	cv::Mat img = imgs[0];
	cv::Mat_<float> tmpsav0 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
	cv::Mat_<float> tmpsav1 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
	...
	for (int i = 0; i*basesize < img.rows; i++)
	{
		for (int j = 0; j*basesize < img.cols; j++)
		{
			...
			cv::Mat_<float> tmpimg = cv::Mat(edi - sti, edj - stj, CV_32FC1);
			for (int c = 0; c < imgs.size(); c++)
			{
				for (int i = sti; i < edi; i++)
				{
					for (int j = stj; j < edj; j++)
					{
						tmpimg(i - sti, j - stj) = imgs[c].at<float>(i, j);
					}
				}
				tmpimgs.push_back(tmpimg.clone());
			}
			...
			if (Caffe::mode() == Caffe::GPU)
			{
				...
				std::vector<GpuMat> output_channels;
				WrapOutputLayer_GPU(&output_channels);
				Preprocess_multipleinput_GPU(tmpimgs, &input_channels);
				net_->Forward();
				for (int i = 0; i < 3; i++)
				{
					cv::Mat tmp;
			...
			else
			{
				...
				std::vector<cv::Mat> output_channels;
				WrapOutputLayer(&output_channels);
				Preprocess_multipleinput(tmpimgs, &input_channels);
				net_->Forward();
				for (int i = 0; i < 3; i++)
				{
					cv::Mat tmp;
...
std::vector<cv::Mat> Interpolator::interpolate_4x_c_otherside(const std::vector<cv::Mat>& imgs)
{
	cv::Mat img = imgs[0];
	cv::Mat_<float> tmpsav1 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
	cv::Mat_<float> tmpsav2 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
...
std::vector<cv::Mat> Interpolator::interpolate_4x_num(const cv::Mat& img, int num)
{
	cv::Mat_<float> tmpsav0 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
	cv::Mat_<float> tmpsav1 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
	cv::Mat_<float> tmpsav2 = cv::Mat_<float>(img.rows, img.cols, CV_32FC1);
	...
	for (int i = 0; i*basesize < img.rows; i++)
	{
		for (int j = 0; j*basesize < img.cols; j++)
		{
			...
			for (int i = sti; i < edi; i++)
			{
				for (int j = stj; j < edj; j++)
				{
					tmpsav0(i, j) = resmats[0].at<float>(i - sti + di, j - stj + dj);
					...
					//tmpsav14(i, j) = resmats[14].at<float>(i - sti + di, j - stj + dj);
				}
			}
