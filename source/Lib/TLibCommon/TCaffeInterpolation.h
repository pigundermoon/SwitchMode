...
public:
	...
	std::vector<cv::Mat>  interpolate_4x_num(const cv::Mat& img,int num);
	std::vector<cv::Mat>  interpolate_4x_c(const cv::Mat& img);
	std::vector<cv::Mat>  interpolate_4x_c_otherside(const std::vector<cv::Mat>& imgs);
	std::vector<cv::Mat>  interpolate_otherside(const std::vector<cv::Mat>& imgs);
...
private:
	...
	void Preprocess_multipleinput(const std::vector<cv::Mat>& img,
		std::vector<cv::Mat>* input_channels);
	...
	void Preprocess_multipleinput_GPU(const std::vector<cv::Mat>& img,
		std::vector<GpuMat>* input_channels);
